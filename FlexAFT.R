...
FlexAFT<-function(Data,Var,NL,TD,nknot.NL,nknot.TD,degree.NL,degree.TD,nknot.bh,degree.bh,Time.Obs,Delta,knot_time="alltime",tol=1e-5,ndivision="maxtime",init.beta="exp",init.gamma="exp",init.alpha="linear"){
  ...
  while(diff>tol){
    ...
    #                    betaVec=betaVec,alphaVec=alphaVec,
    #                    nvar,NL,TD,degree.TD,pos.alpha,pos.beta,
    #                    df.alpha,df.beta,df.gamma,
    #                    Ax,Bst,df.bt,df.X,beta.knots,Var,
    #                    Tt,delta,degree.bh,bh.knots,ndivision,Data,control=list(maxit = 5000,fnscale=-1))
    ...
    #conditional on alpha_hat and gamma_hat from the prevous iteration
    ...
    if (sum(TD)==0 & sum(NL)==nvar){
      ...
      #update_beta<-optim(betaVec,fn=AFT.logLik.beta,gr = AFT.logLik.beta.der, 
      #                   alphaVec=alphaVec,gammaVec=gamma.new,
      #                   nvar,NL,TD,degree.TD,pos.alpha,pos.beta,
      #                   df.alpha,df.beta,df.gamma,
      #                   Ax,Bst,df.bt,df.X,beta.knots,Var,
      #                   Tt,delta,degree.bh,bh.knots,ndivision,Data,
    ...
    if (sum(NL)==0){
      ...
      #update_alpha<-optim(alphaVec,fn=AFT.logLik.alpha,gr = AFT.logLik.alpha.der, 
      #                    betaVec=beta.new,gammaVec=gamma.new,
      #                    nvar,NL,TD,degree.TD,pos.alpha,pos.beta,
      #                    df.alpha,df.beta,df.gamma,
      #                    Ax,Bst,df.bt,df.X,beta.knots,Var,
      #                    Tt,delta,degree.bh,bh.knots,ndivision,Data,
  ...
  #each element in the list corresponds to one covaraite
...
AFT.logLik.alpha<-function(alphaVec
                           #,betaVec,gammaVec,nvar,NL,TD,degree.TD,
                           #pos.alpha,pos.beta,df.alpha,df.beta,df.gamma,
                           #Ax,Bst,df.bt,df.X,beta.knots,Var,
                           #Tt,delta,degree.bh,bh.knots,ndivision,Data
){
  ...
  logL2<-integrat("none"
                  #,df.bt,gx,beta,gamma,df.beta,df.gamma,
                  #Var,Tt,nvar,NL,TD,degree.TD,beta.knots,
                  #degree.bh,bh.knots,ndivision,Data
  )
...
AFT.logLik.beta<-function(betaVec
                          #,alphaVec,gammaVec,nvar,NL,TD,degree.TD,
                          #pos.alpha,pos.beta,df.alpha,df.beta,df.gamma,
                          #Ax,Bst,df.bt,df.X,beta.knots,Var,
                          #Tt,delta,degree.bh,bh.knots,ndivision,Data
){
...
AFT.logLik.gamma<-function(gammaVec
                           #,alphaVec,betaVec,nvar,NL,TD,degree.TD,
                           #pos.alpha,pos.beta,df.alpha,df.beta,df.gamma,
                           #Ax,Bst,df.bt,df.X,beta.knots,Var,
                           #Tt,delta,degree.bh,bh.knots,ndivision,Data
){
...
###it is a computation function concerning the estimation of alpha, beta, gamma, or calculate the loglikelihood function

integrand<-function(u,wrt
                    #,df.bt,gx,beta,gamma,df.beta,df.gamma,
                    #Var,Tt,nvar,NL,TD,degree.TD,beta.knots,
                    #degree.bh,bh.knots,Data
){
  n.sam<-length(Tt)
...
##use numeric integration to compute the cumulative hazard
#caculation is based on matrix multiplication
...
S.integrand<-function(u
                      #,fit,cov,gx.ref,nvar,NL,TD,Tt,Var,W.range,Data
)
{
...
SurvEst<-function(fit,time,cov,Data){
  ...
  yvalue<-S.integrand(xmatrix
                      #,fit=fit,cov=cov,gx.ref=gx.ref,nvar=nvar,
                      #NL=NL,TD=TD,Tt=Tt,Var=Var,W.range=W.range,Data=Data
  )
  #numerical integration by matrix multiplication
  if (length(time)==1){
...
TDTRest<-function(fit,Data,time,cov0,cov1,pct){
  ...
  for (i in 1:length(pct)){
    sq<-pct[i]  
    est.Event.time.ind<-apply(cbind(sq,est.St),1,function(x) {which(abs(x[-1]-x[1])<=0.01)[1]})
...
AFTLoglike<-function(fit,Data,newdata){
  ...
  logL2<-integrat(wrt="none"
                  #,df.bt=df.bt,gx=gx.new,beta=fit$beta,gamma=fit$gamma,
                  #df.beta=df.beta,df.gamma=df.gamma,
                  #Var=Var,Tt=newTt,nvar=nvar,NL=NL,TD=TD,
                  #degree.TD=degree.TD,beta.knots=fit$TD.knots,
                  #degree.bh=fit$degree.bh,bh.knots=fit$bh.knots,ndivision,Data=newdata
  )
